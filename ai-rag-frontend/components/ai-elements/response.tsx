...
const components: Options['components'] = {
	...
	pre: ({ node, className, children }) => {
		...
		return (
			<CodeBlock className={cn('my-4 h-auto', className)} code={(children.props as { children: string }).children} language={language}>
				<CodeBlockCopyButton
...
export const Response = memo(
	({
		...
		const parsedChildren = typeof children === 'string' && shouldParseIncompleteMarkdown ? parseIncompleteMarkdown(children) : children;
